# импортируем данные о пробках 22 октября
import os
import json 
import pandas as pd
df = pd.DataFrame()
...
# делаем столбец с координатами
currentDf['coordinates'] = currentDf['vehicle.position.latitude'].astype(str) + ','+ currentDf['vehicle.position.longitude'].astype(str)



# читаем файл с автобусами и их хаарктеристиками (останвоки по порядку, расстояние между останвоками и др.)
...
# берем все уникальные id из данных всех идущих в это время автобусов
...
for i in listOfId:
    ...
    if len(curId) > 1:
        ...
        commonTime = abs(int(resultCurrent["vehicle.timestamp"].loc[max(indexes)]) - int(resultCurrent["vehicle.timestamp"].loc[min(indexes)]))
...
# newDf.to_csv('2020-10-22.08:00-08:10', index=False)